...
let array = ['A','B','C'];  //  array 선언 안에 abc 인덱스가 있음  
console.log(array); // index 0 = A ,  index 1 = B , index 2 = C  이런식으로 출력됨
console.log(array[2]); // array 라는 상수 안에 index 2 를 호출 하는 거임  그래서 C 가 출력됨
array.push('D','E');   // push 는 맨 뒤에서 부터 넣음
console.log(array);
array.pop(); // pop 은 맨뒤에서 부터 날림!!! ㅇㅋ?
console.log(array);
...
console.log(lyrics.indexOf('[형돈]'));                        // 변수.indexOf('문자열')   ==   index 수 표시함      (결과값은 236 나옴)
console.log(lyrics.indexOf(`[길]`));                         // (결과값 279 나옴)                            사용법 : 문자열.indexOf
hyungdon = lyrics.slice(236,279);                           // 변수.slice(메소드,메소드)  == 잘라내는거임       사용법 : 문자열.slice 
//아래는 최종 결과값임
